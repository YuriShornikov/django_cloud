...
class FileUploadView(viewsets.ViewSet):
    ...
    def upload_file(self, request, *args, **kwargs):
        ...
        comment = request.data.get('comment', '')  # Получаем комментарий из данных запроса
        ...
        for file in files:
            if file.size > max_file_size:  # Проверка на размер файла
    ...
    def delete_file(self, request, user_id=None, file_id=None, *args, **kwargs):
        ...
        if user.avatar == file_instance.url:
            user.avatar = None  # Очищаем поле аватара
    ...
    def update_file(self, request, file_id=None, *args, **kwargs):
        """Переименование файла или обновление комментария."""
        if request.user.is_admin:
            file_instance = get_object_or_404(File, id=file_id)
        else:
            file_instance = get_object_or_404(File, id=file_id, user=request.user)
        ...
        if new_name:
            file_instance.file_name = new_name  # Переименовываем файл
        if new_comment:
            file_instance.comment = new_comment  # Обновляем комментарий
        ...
        file_instance.save()  # Сохраняем изменения

        serializer = FileSerializer(file_instance)  # Сериализуем обновленный файл
        return Response(serializer.data, status=status.HTTP_200_OK)
